...
def parse_assets_xml_assets_tech_nodes(
    assets_xml_path: str | Path, text_map: dict[str, str] | None = None
) -> list[TechNode]:
    ...
    # Pre-scan for TechCategory offsets and membership
    tech_x_offsets: dict[str, int] = {}
    tech_y_offsets: dict[str, int] = {}
    economy_guids: set[str] = set()
    civic_guids: set[str] = set()
    military_guids: set[str] = set()
    dlc01_guids: set[str] = set()
    ...
    for asset in root.iter("Asset"):
        ...
        cat_name = (_text_of(asset.find("Values/Standard/Name")) or "").lower()
        is_economy = "economy" in cat_name
        is_civic = "civic" in cat_name
        is_military = "military" in cat_name
        is_dlc01 = "dlc01" in cat_name
        x_off = 0
        ...
        if x_off != 0 or y_off != 0 or is_economy or is_civic or is_military or is_dlc01:
            for item in asset.findall("Values/TechCategory/Techs/Item"):
                t_guid = _text_of(item.find("Tech"))

                if t_guid:
                    # DLC01 Offsets haben Priorität und werden nicht überschrieben
                    if x_off != 0:
                        if is_dlc01 or t_guid not in tech_x_offsets:
                            tech_x_offsets[t_guid] = x_off

                    if y_off != 0:
                        if is_dlc01 or t_guid not in tech_y_offsets:
                            tech_y_offsets[t_guid] = y_off

                    if is_economy:
                        economy_guids.add(t_guid)

                    if is_civic:
                        civic_guids.add(t_guid)

                    if is_military:
                        military_guids.add(t_guid)

                    if is_dlc01:
                        dlc01_guids.add(t_guid)
    ...
    for asset in root.iter("Asset"):
        ...
        if guid in tech_x_offsets:
            grid_x += tech_x_offsets[guid]

        if guid in tech_y_offsets:
            grid_y += tech_y_offsets[guid]

        # Exklusive Verschiebung: Nur eine Regel darf feuern (DLC01 > Economy > Rest)
        if guid in dlc01_guids:
            if abs(raw_grid_x) % 2 != 0:
                grid_y += 0.5
        elif guid in economy_guids:
             if (raw_grid_x % 2 != 0):
                grid_y += 0.5
        elif guid in military_guids:
            if (raw_grid_x % 2 != 0):
                grid_y += 0.5
        elif guid in civic_guids:
            # x = 0 gets y-0.5
            if raw_grid_x == 0:
                grid_y -= 0.5
...
def _find_first_child_text(parent: ET.Element | None, path: str) -> str | None:
...
class TechTreeWindow(QMainWindow):
    ...
    def _load_and_render(self) -> None:
        ...
        try:
            ...
            nodes = parse_assets_xml_assets_tech_nodes(self._assets_xml_path, text_map)
